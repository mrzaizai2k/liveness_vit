...
class StreamVideo(Process):
    ...
    def run(self):
        ...
        frame_producer = KafkaProducer(bootstrap_servers=[f"{self.kafka_host}:{self.kafka_port}"],
...
if __name__=="__main__":
    ...
    kafka_group = kafka_config['kafka_group']
    
    PRODUCERS = [StreamVideo(video_path="image_test/vid2.mp4", camera_name="samsung", 
                             topic=kafka_topic, 
                             kafka_host = kafka_host, kafka_port=str(kafka_port), 
                             )
                 ]
